...
FOR EACH ROW
BEGIN
    DECLARE cidade_nome VARCHAR(255);
    DECLARE tipo_crime_nome VARCHAR(255);
    
    
    SELECT Nome INTO cidade_nome
    FROM Cidade
    WHERE IdCidade = NEW.IdCidade;
    
    
    SELECT Nome INTO tipo_crime_nome
    FROM TipoCrime
    WHERE IdTipoCrime = NEW.IdTipoCrime;
    
    
    INSERT INTO LogAlteracoesCrime (IdRegistro, TipoOperacao, DescricaoAlteracao)
    VALUES (NEW.IdCrime, 'INSERT', CONCAT('Novo registro adicionado: ', tipo_crime_nome, ' na cidade ', cidade_nome));
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
/*!50003 SET character_set_client  = @saved_cs_client */ ;
...
BEGIN
    ...
    IF OLD.IdCidade != NEW.IdCidade THEN
        INSERT INTO LogAlteracoesCrime (IdRegistro, TipoOperacao, DescricaoAlteracao)
        SELECT NEW.IdCrime, 'UPDATE', 
               CONCAT('Cidade alterada: ', OLD_Cidade.Nome, ' para ', NEW_Cidade.Nome)
        FROM Cidade OLD_Cidade, Cidade NEW_Cidade
        WHERE OLD_Cidade.IdCidade = OLD.IdCidade
          AND NEW_Cidade.IdCidade = NEW.IdCidade;
    END IF;
END */;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
CREATE TABLE `LogAlteracoesCrime` (
  ...
  `TipoOperacao` enum('INSERT','UPDATE','DELETE') NOT NULL,
  `DescricaoAlteracao` text DEFAULT NULL,
  `DataHora` datetime DEFAULT current_timestamp(),
  PRIMARY KEY (`IdLog`),
  KEY `IdRegistro` (`IdRegistro`),
  CONSTRAINT `LogAlteracoesCrime_ibfk_1` FOREIGN KEY (`IdRegistro`) REFERENCES `Crime` (`IdCrime`) ON DELETE CASCADE ON UPDATE CASCADE
...
-- Dump completed on 2024-12-10 21:35:54
